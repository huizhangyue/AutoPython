...
class RequestsHandler:
    ...
    def get_Req(self, url, params, **kw):  # **kwargs是不定长参数，headers是放在这个不定长参数
        '''封装一个get方法，发送get请求'''

        try:  # 当处理不成功时，比如URL地址输入方式错误，或者接口请求超时timeout，需要抛出异常
            res = requests.get(url, params=params, **kw)

        except TimeoutError:
            # 记录日志信息，放入logger里，这样我们就能知道问题出在哪里
            logging.error('访问失败')
